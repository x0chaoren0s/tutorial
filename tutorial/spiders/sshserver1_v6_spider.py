...
# scrapy crawl sshservers1_v6
class SSHServers1Spider_director(scrapy.Spider):
    name = "sshservers1_v6"
...
class SSHServers1Spider_worker(scrapy.Spider):
    name = "_sshservers1_v6"
    fillingForm_interval_secs = 60+2  # 该网站要求60s后再创建一个新用户
    CRAWLED_IDX = 0 # 用于计数已爬取的有效服务器，配合上述 fillingForm_interval_secs 进行暂停
    custom_settings = {
        # 'DOWNLOAD_DELAY': 70, # 该网站要求60s后再创建一个新用户
